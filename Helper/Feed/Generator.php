...
class Generator
{
    ...
    public function execute()
    {
        ...
        foreach ($this->_helperData->getStoreViews() as $storeView) {
            ...
            $products = $this->getVisibleProducts($storeId);

            $this->iterator->walk($products->getSelect(), [function($args) use (
                $siteId, $storeId, $mediaUrl, $currency, $gender_code, $age_group_code, $brand_code, $default_brand, $colour_code, $attribute_codes,
            ) {
                $row = $args['row'];
    ...
    protected function getVisibleProducts($storeId)
    {
